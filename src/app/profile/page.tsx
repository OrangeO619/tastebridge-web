...
import { ArrowLeft, ChevronLeft, ChevronRight, Heart, ImageDown, MapPin, Search, Sparkles, Star, X } from "lucide-react";
...
export default function ProfilePage(){
  ...
  const[previewImages,setPreviewImages]=useState<string[]>([]);const[previewIndex,setPreviewIndex]=useState(0);
  const openPreview=(images:string[],index:number)=>{setPreviewImages(images);setPreviewIndex(index);};
...
const shownName=localName||displayName;
  ...
  return(<div className="min-h-dvh bg-gradient-to-br from-zinc-900 via-stone-900 to-zinc-800">
    <div className="sticky top-0 z-10 flex items-center gap-3 bg-black/40 px-4 py-3 backdrop-blur-md">
      ...
      <button onClick={signOut} className="rounded-full bg-white/10 px-2.5 py-1 text-xs text-white/70 hover:bg-white/20 cursor-pointer">退出</button>
      <div className="flex items-center gap-2"><button onClick={exportXLSX} className="rounded-full bg-white/10 px-2.5 py-1 text-xs text-emerald-400 hover:bg-white/20 cursor-pointer">导出 XLSX</button><button onClick={openMonthlySummary} className="rounded-full bg-amber-500/25 px-2.5 py-1 text-xs text-amber-200 hover:bg-amber-500/35"><span className="inline-flex items-center gap-1 cursor-pointer"><Sparkles className="h-3 w-3"/>本月回顾</span></button><Link href="/together" className="rounded-full bg-sky-500/20 px-2.5 py-1 text-xs text-sky-200 hover:bg-sky-500/30"><span className="inline-flex items-center gap-1"><Heart className="h-3 w-3"/>共同回忆</span></Link></div>
    ...
    <div className="mx-auto max-w-lg px-4 pb-12 pt-4"><div className="mb-5 flex items-center gap-4 rounded-2xl bg-white/8 p-4 ring-1 ring-white/10"><input ref={avatarInputRef} type="file" accept="image/*" className="hidden" onChange={e=>{const f=e.target.files?.[0];if(f)uploadAvatar(f);}}/><button type="button" onClick={()=>{if(editMode)avatarInputRef.current?.click();}} className={`relative flex-shrink-0${editMode?" cursor-pointer":" cursor-default"}`}>{(localAvatarUrl||avatarUrl)?<img src={(localAvatarUrl||avatarUrl)??""} alt="" className="h-16 w-16 rounded-full object-cover ring-2 ring-amber-500/40"/>:<span className="flex h-16 w-16 items-center justify-center rounded-full bg-amber-500 text-2xl font-bold text-white">{shownName.slice(0,1).toUpperCase()}</span>}{editMode&&<span className="absolute inset-0 flex items-center justify-center rounded-full bg-black/40 text-[10px] text-white">{uploadingAvatar?"上传中…":"换头像"}</span>}</button>{!editMode?(<div className="flex-1 min-w-0"><p className="truncate text-lg font-bold text-white">{shownName}</p><p className="truncate text-xs text-white/40">{user?.email}</p><button onClick={()=>{setEditName(shownName);setEditMode(true);}} className="mt-2 rounded-full bg-white/10 px-3 py-1 text-xs text-white/70 hover:bg-white/20 cursor-pointer">编辑资料</button></div>):(<div className="flex-1 space-y-2"><input value={editName} onChange={e=>setEditName(e.target.value)} placeholder="显示名称" className="w-full rounded-xl bg-white/10 px-3 py-1.5 text-sm text-white placeholder-white/30 outline-none ring-1 ring-white/20 focus:ring-amber-500"/>{editErr&&<p className="text-xs text-red-400">{editErr}</p>}<div className="flex gap-2"><button onClick={saveProfile} disabled={editSaving} className="flex-1 rounded-xl bg-amber-500 py-1.5 text-xs font-semibold text-white disabled:opacity-50">{editSaving?"保存中…":"保存"}</button><button onClick={()=>setEditMode(false)} className="rounded-xl bg-white/10 px-4 py-1.5 text-xs text-white/70">取消</button></div></div>)}</div>
      ...
      <div className="flex flex-col gap-3">
        {filtered.map(({pref,spot})=>(<div key={pref.id} className="overflow-hidden rounded-2xl bg-white/8 ring-1 ring-white/10">
          {pref.images&&pref.images.length>0&&(<div className="flex gap-1.5 overflow-x-auto p-2 pb-0">{pref.images.map((url,i)=>(<button key={url} type="button" onClick={()=>openPreview(pref.images!,i)} className="flex-shrink-0"><img src={url} alt="" className="h-28 w-28 rounded-xl object-cover hover:opacity-80 transition-opacity cursor-zoom-in"/></button>))}</div>)}
          <div className="flex items-start gap-3 p-4">
            ...
            <button onClick={()=>handleDelete(pref.id)} className="ml-2 rounded-full bg-red-900/30 px-2 py-0.5 text-[10px] text-red-300 hover:bg-red-900/50 cursor-pointer">删除</button>
          </div>
    ...
    {previewImages.length>0&&(<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90" onClick={closePreview}><button type="button" onClick={closePreview} className="absolute right-4 top-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20"><X className="h-5 w-5"/></button>{previewImages.length>1&&(<button type="button" onClick={e=>{e.stopPropagation();prevImage();}} className="absolute left-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20"><ChevronLeft className="h-6 w-6"/></button>)}<img src={previewImages[previewIndex]} alt="" className="max-h-[85vh] max-w-[90vw] rounded-lg object-contain" onClick={e=>e.stopPropagation()}/>{previewImages.length>1&&(<button type="button" onClick={e=>{e.stopPropagation();nextImage();}} className="absolute right-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20"><ChevronRight className="h-6 w-6"/></button>)}{previewImages.length>1&&(<div className="absolute bottom-6 left-1/2 -translate-x-1/2 rounded-full bg-black/50 px-3 py-1 text-sm text-white">{previewIndex+1} / {previewImages.length}</div>)}</div>)}
  </div>);
}
